...
public class Lotto {
	...
	public void setLotto() {
		for (int i = 0; i < lotto.length; i++) {
			lotto[i] = 0; 	// 배열내부의 지역변수 6개를 for 문을 이용해서 0으로 초기화하는 로직
		}
		for (int i = 0; i < lotto.length; i++) {
			// 로또의 사이즈 6만큼 회전하면서 랜덤숫자 6개를 생성
			int randomNum = (int) ((Math.random()*45)+1);
			// 로또의 법칙 : 먼저나온 숫자가 중복이 있어서는 안된다.
			// 나중에 나온 숫자가 먼저 나온 숫자와 같다면 그 숫자는 폐기하고 다시 돌아야 한다.
			// 이때, 카운트 숫자는 증가하면 안된다.
			boolean exist = false;
			// for 문 내부에 다시 for 문을 주어(이중for문) 번호의 중복여부를 체크하고 만약 중복되었다면
			// 회전카운트(i) 값을 마이너스 처리하여 이번 회전을 무효화 시킨다.
